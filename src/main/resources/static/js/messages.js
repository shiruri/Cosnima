...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Check for rental action from listing page
  const rentalAction = params.get('action');
  const listingId = params.get('listingId');
  if (preselect && rentalAction === 'rent' && listingId) {
    setTimeout(() => {
      openRentalRequestInChat(preselect, listingId);
    }, 500);
  }
...
function renderChatHeader(convo) {
  ...
  if (otherId) {
    API.get(`/api/users/${otherId}/public`, true).then(user => {
      if (user && nameEl) {
...
async function openRentalRequestInChat(convoId, listingId) {
  // Get listing details for the rental request
  try {
    const listing = await API.get(`/api/listings/${listingId}`, true);
    if (!listing) return;

    // Show rental modal or send rental card directly
    const rentalPrompt = confirm('Would you like to send a rental request for this item? Click OK to proceed with rental dates.');
    
    if (rentalPrompt) {
      // Open rental date selection (reuse the listing-actions modal)
...
async function submitRentFromChat(listingId, sellerId) {
  ...
  // Check availability
  try {
    const avail = await API.get(`/api/listings/${listingId}/availability?startDate=${start}&endDate=${end}`, false);
    if (avail === false) {
  ...
  try {
    // Create rental (backend will create and possibly send a message)
...
function openOfferInChat() {
  ...
  modal.innerHTML = `
    <div class="offer-modal-backdrop" style="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:999;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px);">
      <div class="offer-modal-card" style="background:var(--card);border-radius:var(--radius-xl);width:min(420px,90%);max-width:420px;box-shadow:var(--shadow-lg);overflow:hidden;">
        <div class="offer-modal-header" style="background:linear-gradient(135deg,var(--accent),var(--accent-dark));padding:var(--space-lg);color:white;">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <h3 style="margin:0;font-size:1.1rem;">Make an Offer</h3>
            <button onclick="document.getElementById('offer-modal').remove()" style="background:none;border:none;color:white;font-size:1.5rem;cursor:pointer;opacity:0.8;">&times;</button>
          </div>
        </div>
        <div class="offer-modal-body" style="padding:var(--space-lg);">
          <div class="offer-listing-preview" style="display:flex;align-items:center;gap:var(--space-md);padding:var(--space-md);background:var(--bg-alt);border-radius:var(--radius);margin-bottom:var(--space-lg);">
          ...
          <div class="offer-field" style="margin-bottom:var(--space-md);">
            <label style="display:block;font-size:0.75rem;font-weight:800;text-transform:uppercase;letter-spacing:0.05em;color:var(--ink-faint);margin-bottom:var(--space-xs);">Your Offer (₱) *</label>
            <input type="number" id="offer-price-input" placeholder="Enter your offer amount" min="1" step="1" style="width:100%;padding:0.75rem;border:2px solid var(--border);border-radius:var(--radius);font-size:1rem;font-weight:600;color:var(--ink);background:var(--bg);outline:none;transition:border-color var(--t-fast);">
            <div id="offer-price-error" class="offer-field-error" style="font-size:0.72rem;color:var(--error);margin-top:4px;display:none;">Please enter a valid price greater than ₱0.</div>
          </div>
          
          <div class="offer-field" style="margin-bottom:var(--space-md);">
            <label style="display:block;font-size:0.75rem;font-weight:800;text-transform:uppercase;letter-spacing:0.05em;color:var(--ink-faint);margin-bottom:var(--space-xs);">Message (Optional)</label>
            <textarea id="offer-message-input" placeholder="Add a message to the seller..." rows="3" style="width:100%;padding:0.75rem;border:2px solid var(--border);border-radius:var(--radius);font-size:0.9rem;color:var(--ink);background:var(--bg);outline:none;resize:vertical;font-family:var(--font-body);"></textarea>
          </div>
          
          <div id="offer-status" style="display:none;padding:0.75rem;border-radius:var(--radius);font-size:0.84rem;font-weight:700;margin-bottom:var(--space-md);"></div>
        </div>
        <div class="offer-modal-footer" style="padding:var(--space-lg);border-top:1px solid var(--border);display:flex;gap:var(--space-sm);justify-content:flex-end;">
          <button class="btn btn-ghost" onclick="document.getElementById('offer-modal').remove()" style="padding:0.6rem 1.2rem;">Cancel</button>
          <button class="btn btn-primary" id="submit-offer-btn" onclick="submitOfferInChat()" style="padding:0.6rem 1.2rem;background:var(--accent);">
...
async function submitOfferInChat() {
  ...
  const btnText = btn?.querySelector('.btn-text');
  const btnLoader = btn?.querySelector('.btn-loader');
  const errorEl = document.getElementById('offer-price-error');
...
function openRentInChat() {
  ...
  // Remove existing
  document.getElementById('rent-modal')?.remove();
  
  const modal = document.createElement('div');
  modal.id = 'rent-modal';
  modal.innerHTML = `
    <div class="rent-modal-backdrop" style="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:999;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px);">
      <div class="rent-modal-card" style="background:var(--card);border-radius:var(--radius-xl);width:min(420px,90%);max-width:420px;box-shadow:var(--shadow-lg);overflow:hidden;">
        <div class="rent-modal-header" style="background:linear-gradient(135deg,var(--beaver),var(--beaver-dark));padding:var(--space-lg);color:white;">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <h3 style="margin:0;font-size:1.1rem;">Request Rental</h3>
            <button onclick="document.getElementById('rent-modal').remove()" style="background:none;border:none;color:white;font-size:1.5rem;cursor:pointer;opacity:0.8;">&times;</button>
          </div>
        </div>
        <div class="rent-modal-body" style="padding:var(--space-lg);">
          <div class="rent-listing-preview" style="display:flex;align-items:center;gap:var(--space-md);padding:var(--space-md);background:var(--bg-alt);border-radius:var(--radius);margin-bottom:var(--space-lg);">
            <div style="width:60px;height:60px;border-radius:var(--radius);overflow:hidden;background:var(--border);flex-shrink:0;">
              ${listing.images && listing.images[0] ? `<img src="${listing.images[0].imageUrl}" style="width:100%;height:100%;object-fit:cover;">` : ''}
            </div>
            <div style="flex:1;min-width:0;">
              <div style="font-weight:700;color:var(--ink);font-size:0.9rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escH(listing.title)}</div>
              <div style="font-size:0.8rem;color:var(--ink-muted);">₱${Number(listing.price || 0).toLocaleString()} / day</div>
            </div>
          </div>
          
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-md);margin-bottom:var(--space-md);">
            <div class="rent-field">
              <label style="display:block;font-size:0.75rem;font-weight:800;text-transform:uppercase;letter-spacing:0.05em;color:var(--ink-faint);margin-bottom:var(--space-xs);">Start Date *</label>
              <input type="date" id="rent-start-input" min="${new Date().toISOString().split('T')[0]}" style="width:100%;padding:0.75rem;border:2px solid var(--border);border-radius:var(--radius);font-size:0.9rem;color:var(--ink);background:var(--bg);">
            </div>
            <div class="rent-field">
              <label style="display:block;font-size:0.75rem;font-weight:800;text-transform:uppercase;letter-spacing:0.05em;color:var(--ink-faint);margin-bottom:var(--space-xs);">End Date *</label>
              <input type="date" id="rent-end-input" min="${new Date().toISOString().split('T')[0]}" style="width:100%;padding:0.75rem;border:2px solid var(--border);border-radius:var(--radius);font-size:0.9rem;color:var(--ink);background:var(--bg);">
            </div>
          </div>
          
          <div class="rent-field" style="margin-bottom:var(--space-md);">
            <label style="display:block;font-size:0.75rem;font-weight:800;text-transform:uppercase;letter-spacing:0.05em;color:var(--ink-faint);margin-bottom:var(--space-xs);">Deposit (₱)</label>
            <input type="number" id="rent-deposit-input" placeholder="Optional deposit amount" min="0" style="width:100%;padding:0.75rem;border:2px solid var(--border);border-radius:var(--radius);font-size:0.9rem;color:var(--ink);background:var(--bg);">
          </div>
          
          <div id="rent-summary" style="display:none;padding:var(--space-md);background:var(--bg-alt);border-radius:var(--radius);margin-bottom:var(--space-md);">
            <div style="display:flex;justify-content:space-between;font-size:0.85rem;margin-bottom:var(--space-xs);">
              ...
              <span id="rent-duration" style="font-weight:700;color:var(--ink);">—</span>
            </div>
            <div style="display:flex;justify-content:space-between;font-size:0.9rem;border-top:1px solid var(--border);padding-top:var(--space-xs);margin-top:var(--space-xs);">
              <span style="color:var(--ink);font-weight:700;">Total</span>
              <span id="rent-total" style="font-family:var(--font-display);color:var(--beaver);font-weight:700;">—</span>
            </div>
          </div>
          
          <div id="rent-status" style="display:none;padding:0.75rem;border-radius:var(--radius);font-size:0.84rem;font-weight:700;margin-bottom:var(--space-md);"></div>
        </div>
        <div class="rent-modal-footer" style="padding:var(--space-lg);border-top:1px solid var(--border);display:flex;gap:var(--space-sm);justify-content:flex-end;">
          <button class="btn btn-ghost" onclick="document.getElementById('rent-modal').remove()" style="padding:0.6rem 1.2rem;">Cancel</button>
          <button class="btn btn-primary" id="submit-rent-btn" onclick="submitRentInChat()" style="padding:0.6rem 1.2rem;background:var(--beaver);">
            <span class="btn-text">Send Request</span>
            <span class="btn-loader" style="display:none;width:14px;height:14px;border:2px solid rgba(255,255,255,0.5);border-top-color:white;border-radius:50%;animation:spin 0.6s linear infinite;"></span>
          </button>
        </div>
  ...
  // Wire up price calculation
  const startInput = document.getElementById('rent-start-input');
  const endInput = document.getElementById('rent-end-input');
  const depositInput = document.getElementById('rent-deposit-input');
  ...
  const calcPrice = () => {
    ...
    if (start && end) {
      ...
      if (endDate > startDate) {
        ...
        document.getElementById('rent-duration').textContent = `${days} day${days > 1 ? 's' : ''}`;
        document.getElementById('rent-total').textContent = `₱${Number(total).toLocaleString()}`;
        document.getElementById('rent-summary').style.display = 'block';
      }
    }
  };
...
async function submitRentInChat() {
  ...
  const endInput = document.getElementById('rent-end-input');
  const depositInput = document.getElementById('rent-deposit-input');
  const btn = document.getElementById('submit-rent-btn');
  const btnText = btn?.querySelector('.btn-text');
  const btnLoader = btn?.querySelector('.btn-loader');
  ...
  try {
    ...
    if (statusEl) {
      statusEl.textContent = err?.message || 'Could not send request. Try again.';
      statusEl.style.display = 'block';
      statusEl.style.background = 'rgba(192,57,43,0.1)';
      statusEl.style.color = 'var(--error)';
    }
